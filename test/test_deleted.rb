...
conn.transaction do |tx|
  load_osm_file("#{File.dirname(__FILE__)}/test_deleted_nodes.osm", tx)
  load_osm_file("#{File.dirname(__FILE__)}/test_deleted_ways.osm", tx)
  load_osm_file("#{File.dirname(__FILE__)}/test_deleted_relations.osm", tx)
end

# bad requests:
['/api/0.6/deleted',                          # no bbox parameter
 '/api/0.6/deleted?bbox=',                    # empty bbox parameter
 '/api/0.6/deleted?bbox=0,0,0',               # not enough numbers
 '/api/0.6/deleted?bbox=0.1,0.1,-0.1,-0.1',   # negative size bbox
 '/api/0.6/deleted?bbox=179.9,0,180.1,0.1',   # bbox out of valid range
 '/api/0.6/deleted?bbox=-180.1,0,-179.9,0.1', # bbox out of valid range
 '/api/0.6/deleted?bbox=0,89.9,0.1,90.1',     # bbox out of valid range
 '/api/0.6/deleted?bbox=0,-90.1,0.1,-89.9'    # bbox out of valid range
].each do |req|
...
test_request('GET', '/api/0.6/deleted?bbox=-0.01,-0.01,-0.0005,-0.0005') do |headers, data|
  ...
  assert(headers["Content-Disposition"], "attachment; filename=\"deleted.osm\"", "Response content disposition.")
...
test_request('GET', '/api/0.6/deleted?bbox=-0.0005,-0.0005,0.0005,0.0005') do |headers, data|
  assert(headers["Status"], "200 OK", "Response status code.")
  assert(headers["Content-Type"], "text/xml; charset=utf-8", "Response content type.")
  ...
  assert(node.name, "node", "Name of <osm> child.")
  assert(node["id"].to_i, 1, "ID of node")
  assert(node["lat"], nil, "Latitude attribute")
  assert(node["lon"], nil, "Longitude attribute")
  assert(node["user"], "user_1", "User name")
  assert(node["uid"].to_i, 1, "User ID")
  assert(node["visible"], "false", "Visibility attribute")
  assert(node["version"].to_i, 1, "Version attribute")
  assert(node["changeset"].to_i, 1, "Changeset ID")
  assert(node["timestamp"], "2013-02-16T19:02:00Z", "Timestamp")
end
